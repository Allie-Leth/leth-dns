...
# ───────── password ─────────
echo
V[PIHOLE_PASSWORD]=$(openssl rand -base64 18)          # used by both Pi-hole & exporter
echo "Generated password: ${V[PIHOLE_PASSWORD]}"; line

# convert to new FTLCONF var names for .env
V[FTLCONF_webserver_api_password]="${V[PIHOLE_PASSWORD]}"
V[FTLCONF_dns_upstreams]="${V[UPSTREAM_DNS]}"
V[FTLCONF_misc_etc_dnsmasq_d]="true"
mkdir -p etc-pihole etc-dnsmasq.d unbound

# ───────── preview & confirm ─────────
echo ".env will contain:"; for k in "${!V[@]}"; do printf '%-32s = %s\n' "$k" "${V[$k]}"; done; line
...
echo "address=/${DNS_UI_ALIAS}/${HOST_IP:-127.0.0.1}" > etc-dnsmasq.d/02-local-${DNS_UI_ALIAS}.conf
...
echo "Username   : admin"
echo "Password   : ${V[PIHOLE_PASSWORD]}"
line
